...
def main(config_fn):
  ...
  dir_in, dir_left, dir_right, dir_bc, dir_merged = [config.get('Dirs',q) for q in ['dir_in', 'dir_left', 'dir_right', 'dir_bc', 'dir_merged']]
  ...
  #pool = mp.Pool(processes=int(config.get('Params', 'POOL_SIZE')))
  num_libs = count_lines(config.get('Files', 'barcode_fn'))
  #compare_mats = pool.map(partial(one_fileset, cfg = config, num_libs = num_libs), fns_list)
  compare_mats = [partial(one_fileset, cfg = config, num_libs = num_libs)(f) for f in fns_list]

  final_compare_mat = np.sum(np.stack(compare_mats), axis = 0)
  np.savetxt(config.get('Files', 'assign_summary_fn'), final_compare_mat, delimiter = ',')
  merge_all(dir_merged, config.get('Files', 'raw_merged_fn'))
  filter_lengths(config.get('Files', 'raw_merged_fn'), config.get('Files', 'final_merged_fn'), int(config.get('Params','SEQ_LEN')), config.get('Params','LEFT_ONLY'))

def one_fileset(fns, cfg, num_libs):
  file_in, fail_left, fail_right, fail_bc, merged = fns
  cl = -1
  tl = count_lines(file_in)
  lines_per_read = int(cfg.get('Params', 'LINES_PER_READ'))
  aligner = inexact_aligner(cfg, file_in, num_libs)
...
class inexact_aligner(object):
  def __init__(self, cfg, file_in, num_libs):
    self.cfg = cfg
    ...
    self.assn_obj = assn_module.bin_assigner(cfg.get('Files', 'barcode_fn'))
