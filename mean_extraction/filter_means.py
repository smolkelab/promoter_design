...
def munge_one_line(l, seq_id, mins, maxes, m_range, take_mean):
  l = l.strip().split(',')
  seq = l[0]
...
def main(fn_in, fn_out, fn_fail, mins, maxes, m_range, take_mean, outnames):
  ...
  with open(fn_in, 'r') as fi, open(fn_out, 'w') as fo, open(fn_fail, 'w') as fx:
    ...
    for l in fi:
        munged_l = munge_one_line(l, seq_id, mean_ids)
        if munged_l == None:
          fx.write(l)
		  unmungeables += 1
        else:
...
if __name__ == '__main__':
  ...
  maxes = [float(q) for q in config.get('Params','min').strip().split(',')]
