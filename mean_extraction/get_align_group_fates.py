...
class fate_summary(object):
  def __init__(cfg):
    ...
    def _new_record(self, id):
      self.keyset.add(id)
      self.records[id] = {'reads_aln':0, 'reads_N':0, 'reads_ambig':0}
    ...
    def lines_to_reads(self, fn, key_id):
      with open(fn, 'r') as fi:
        for l in fi:
          ...
          self.records[id][key_id] = self.records[id][key_id] + 1
    ...
    def lines_to_groups(self, fn, key_id): # group ID is field 1, 2 on up are reads (sum these)
      with open(fn, 'r') as fi:
        for l in fi:
          ...
          if gp_id not in self.keyset:
            self._new_record(gp_id)
          self.records[id][key_id] = self.records[id][key_id] + num_reads         
