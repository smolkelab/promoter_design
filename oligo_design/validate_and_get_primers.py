...
class simulate_gg(object):
  ...
  def primer_search_pcr(template, is_fwd):
    if is_fwd:
...
def main(cfg)
  ...
  simulator = simulate_gg(fwd_primer_cfg, rev_primer_cfg, oligo_design_cfg)
  sim_df = simulate_oligo_file(fn_in, simulator)
  validate_and_generate_oligo_fasta(sim_df, fn_out)

if __name__ == '__main__':
  cfg = ConfigParser.RawConfigParser(allow_no_value=True); ans.optionxform=str
  cfg.read(sys.argv[1])
  main(cfg)
