...
# Try to use 'lazy_alignment' to align clusters. Write failed clusters to a separate file ('file_hard').
  ...
  with open(file_aln, 'w') as fa, open(file_N, 'w') as fN, open(file_ambig, 'w') as fam:
    with open(file_in) as fi:
      ...
      while(start_next != None):
        ...
        if n_lines != None:
          progress(lines_done, n_lines)

#def main_method(file_in, file_out):
