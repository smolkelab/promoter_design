...
def process_one_sequence(seq_id, source_file, dir_out, do_double = True):
...
def main(table_fn, dir_out):
  with open(table_fn, 'r') as fi:
    for l in fi:
      ...
      if len(tmp) == 1:
        main_fullfile_onlysingle(tmp[0], dir_out)

def main_fullfile_onlysingle(fn_in, dir_out):
  source_file = os.path.expanduser(fn_in)
  with open(source_file, 'r') as fi:
    lines = fi.readlines()[1:]
  num_seqs = len(lines)
  for i in range(num_seqs):
    process_one_sequence(i, source_file, dir_out, do_double = False)
