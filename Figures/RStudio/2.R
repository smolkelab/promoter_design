...
# 2A
setwd('D:/Promoter Design Data/Preliminary Model Logs/')
png(filename = 'D:/Promoter Design Data/Figures/PNGs/2A.png',
    units = 'cm', res = 600, width = fig.w, height = fig.h)
lines = data.frame(process.raw.log('train_GPD_clean.txt')); lines$Epoch = 1:nrow(lines)
lines = melt(lines, id.vars = 'Epoch')
setnames(lines, c('value','variable'), c('Loss', 'Dataset'))
p=ggplot(data=lines, aes(x=Epoch, y = Loss, group = Dataset, color = Dataset)) + geom_line() + 
  geom_vline(xintercept = max(lines$Epoch) - 5, lty = 2) + theme_bw() + 
  ...
  labs(x = 'Epoch', y = 'pGPD Model Loss')
print(p)
dev.off()
...
p = ggplot(dat.mer.use, aes(x = Strength.A, y = Strength.B, color = `Cell Count`)) + 
  ...
  theme(axis.text = element_text(size=ax.text), axis.title = element_text(size=ax.titl, face='bold'),
        legend.text = element_text(size=lg.text), legend.title = element_text(size=lg.titl, face='bold'),
        legend.key.size = unit(lg.size,'cm')) + 
  labs(x = 'Activity (pGPD)', y='Predicted Activity (pGPD)', color = 'Cell\nCount') +
  scale_color_viridis()
print(p)
dev.off()
...
png(filename = 'D:/Promoter Design Data/Figures/PNGs/2C.png',
    units = 'cm', res = 600, width = fig.w, height = fig.h)
lines = data.frame(process.raw.log('train_ZEV_clean.txt')); lines$Epoch = 1:nrow(lines)
lines = melt(lines, id.vars = 'Epoch')
setnames(lines, c('value','variable'), c('Loss', 'Dataset'))
p = ggplot(data=lines, aes(x=Epoch, y = Loss, group = Dataset, color = Dataset)) + geom_line() + 
  geom_vline(xintercept = max(lines$Epoch) - 5, lty = 2) + theme_bw() + 
  theme(axis.text = element_text(size=ax.text), axis.title = element_text(size=ax.titl, face='bold'),
        legend.text = element_text(size=lg.text), legend.title = element_text(size=lg.titl, face='bold'),
        legend.key.size = unit(lg.size,'cm')) + 
  labs(x = 'Epoch', y = 'pZEV Model Loss')
print(p)
dev.off()
...
dat.mer.use$Density = get_density(dat.mer.use$Strength.A, dat.mer.use$Strength.B)
dat.mer.use$`Cell Count` = dat.mer.use$Density
png(filename = 'D:/Promoter Design Data/Figures/PNGs/2E.png',
    units = 'cm', width = fig.w, height = fig.h, res = 600)
...
dat.mer.use$Density = get_density(dat.mer.use$Strength.A, dat.mer.use$Strength.B)
dat.mer.use$`Cell Count` = dat.mer.use$Density
png(filename = 'D:/Promoter Design Data/Figures/PNGs/2F.png',
    units = 'cm', width = fig.w, height = fig.h, res = 600)
