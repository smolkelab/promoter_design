...
# track which rows we use for Source Data
dat$idx = 1:nrow(dat)
...
# 4A
# propagate Rx raw measurements for Source Data
dat.use.a = data.frame(FS = dat$Means_A, VYB = dat$VYB_Mean_A, ymin = dat$bar.min.a, ymax = dat$bar.max.a, 
                       Promoter = dat$Promoter, rep = 'A', Offscale = !(dat$trusted_a), idx = dat$idx)
...
dat.use.b = data.frame(FS = dat$Means_B, VYB = dat$VYB_Mean_B, ymin = dat$bar.min.b, ymax = dat$bar.max.b,
                       Promoter = dat$Promoter, rep = 'B', Offscale = !(dat$trusted_b), idx = dat$idx)
...
# Source Data
dat.source = dat[dat.use$idx,]
dat.source = data.table(Seqs = dat.source$Seqs, Type = dat.use$Type, FS = dat.use$FS,
                        R1_TRUE = dat.source$R1_TRUE, R2_TRUE = dat.source$R2_TRUE, R3_TRUE = dat.source$R3_TRUE,
                        R1_FALSE = dat.source$R1_FALSE, R2_FALSE = dat.source$R2_FALSE, R3_FALSE = dat.source$R3_FALSE)
dat.source = dat.source[order(dat.source$Type, dat.source$FS),]
write.csv(dat.source, 'D:/Promoter Design Data/Source Data/4A_raw.csv', quote = FALSE, row.names = FALSE)
...
p = ggplot(dat.use, aes(x = FS, y = VYB, color = Type, ymin = ymin, ymax = ymax)) + 
  ...
  theme(axis.text = element_text(size=6), axis.title = element_text(size=8, face='bold'),
        ...
        legend.key.size = unit(0.3,'cm')) + # was 0.2 cm
  ...
  scale_color_manual(values=c("#440154FF", "#2D708EFF", "#55C667FF")) + 
...
png(filename = 'D:/Promoter Design Data/Figures/PNGs/4B.png',
    units = 'cm', width = w, height = h + 0.4, res = 600)
...
# Source Data
dat.zev.i.all$Name = factor(dat.zev.i.all$Name, 
                            levels = dat.zev.i.all$Name[order(dat.zev.i.all$Experiment.designed, dat.zev.i.all$VYB_Mean_B)])
dat.source = dat.zev.i.all[order(dat.zev.i.all$Name),]
dat.source = data.frame(R1_TRUE = dat.source$R1_TRUE, R2_TRUE = dat.source$R2_TRUE, R3_TRUE = dat.source$R3_TRUE,
                        Name=c(paste0('pZEV-I-', 1:11), 'P8', 'P4', 'P3'))
write.csv(dat.source, 'D:/Promoter Design Data/Source Data/4C_raw.csv', quote = FALSE, row.names = FALSE)
...
# Source Data
dat.source = dat.zev.ar[order(dat.zev.ar$Name),]
dat.source = data.frame(R1_TRUE = dat.source$R1_TRUE, R2_TRUE = dat.source$R2_TRUE, R3_TRUE = dat.source$R3_TRUE,
                        R1_FALSE = dat.source$R1_FALSE, R2_FALSE = dat.source$R2_FALSE, R3_FALSE = dat.source$R3_FALSE,
                        Name=xstr)
write.csv(dat.source, 'D:/Promoter Design Data/Source Data/4D_raw.csv', quote = FALSE, row.names = FALSE)
...
png(filename = 'D:/Promoter Design Data/Figures/PNGs/4D.png',
    units = 'cm', width = w, height = h + 0.6, res = 600)
